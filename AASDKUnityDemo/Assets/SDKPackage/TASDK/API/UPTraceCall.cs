...
namespace UPTrace
{
    public class UPTraceCall
    {
            ...
            [DllImport("__Internal")]
			private static extern void getABTestDataWithPlayerIdForIos(string gameName, string funName, string playerId);
			...
			private readonly static string JavaClassStaticMethod_DisableAccessPrivacyInformation =
 "disableAccessPrivacyInformation";
        ...
        // added in 4002
			...
			private readonly static string JavaClassStaticMethod_LogReportWithServerAndExtraJson =
 "logReportWithServerAndExtraJson";
        ...
        public void logReportPlayIdForIap(string playerId, string receiptDataString, bool isbase64,
            Dictionary<string, string> dic)
        {
            if (null == playerId || playerId.Length == 0)
            {
                Logger.LogError(
                    "===> UPTraceCall.logReportPlayIdForIap(), error: the value of parameter playerId is null or empty.");
                return;
            }
            ...
            {
                Logger.LogError(
                    "===> UPTraceCall.logReportPlayIdForIap(), error: the value of parameter receiptDataString is null or empty.");
                return;
            }
#if UNITY_IOS && !UNITY_EDITOR
        ...
        public void logReportServerForIap(string playerId, string gameAccountServer, string receiptDataString,
            bool isbase64, Dictionary<string, string> dic)
        {
            if (null == playerId || playerId.Length == 0)
            {
                Logger.LogError(
                    "===> UPTraceCall.logReportServerForIap(), error: the value of parameter playerId is null or empty.");
                return;
            }
            ...
            {
                Logger.LogError(
                    "===> UPTraceCall.logReportServerForIap(), error: the value of parameter gameAccountServer is null or empty.");
                return;
            }
            ...
            {
                Logger.LogError(
                    "===> UPTraceCall.logReportServerForIap(), error: the value of parameter receiptDataString is null or empty.");
                return;
            }
#if UNITY_IOS && !UNITY_EDITOR
				string jsonmap = dicationaryToString (dic);
        ...
        public void initTtraceSDKWithCallback(string productId, string channelId, Action<string> success,
            Action<string> fail)
        {
            if (null == productId || productId.Length == 0)
            {
                Logger.LogError(
                    "===> UPTraceCall.initTtraceSDK(), error: the value of parameter productId is null or empty.");
                return;
            }
            ...
            {
                Logger.LogError(
                    "===> UPTraceCall.initTtraceSDK(), error: the value of parameter channelId is null or empty.");
                return;
            }
        ...
        public void logReport(string gameAccountId, string iabPurchaseOriginalJson, string iabPurchaseSignature,
            Dictionary<string, string> dic)
        {
            if (gameAccountId == null)
            {
        ...
        public void getABTestDataWithPlayerId(Action<string> success, Action<string> fail, string playerId)
        {
            Logger.LogInfo("getABTestDataWithPlayerId in tracecall");
            // 设置callback回调
            UPTraceObject.getInstance().setGetABTestDataCallback(success, fail);
            // 调用原生的方法
#if UNITY_IOS && !UNITY_EDITOR
				getABTestDataWithPlayerIdForIos(UPTraceObject.Unity_Callback_Class_Name,UPTraceObject.Unity_Callback_Function_Name,playerId);
#elif UNITY_ANDROID && !UNITY_EDITOR
			if (jc != null) {
				jc.CallStatic (JavaClassStaticMethod_GetABTestData,playerId);
			}
#endif
        }
